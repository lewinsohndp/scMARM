...
if __name__ == "__main__":
    ...
    for i in np.arange(.1,.2,.1):
        data_path = "simulations/splat_" + str(i) + "_de/counts.csv"
        meta_path = "simulations/splat_" + str(i) + "_de/meta.csv"
        ...
        datasets.append((X,y))
        dataset_names.append(str(i))

    maskings = [.7, .8, .9, .95, .99]
    arm = LabelProp("configs/semi_basic.txt", 10)
    results = test_label_prop(datasets, maskings, arm, epochs=100)

    x = 0
    for dataset_results in results:
        dir_path = output_path + "dataset_" + dataset_names[x]
